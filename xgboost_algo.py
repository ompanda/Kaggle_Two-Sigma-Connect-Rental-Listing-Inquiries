...
images = sorted(glob.glob('./data/images_sample/**/**.jpg'))
for im in images:
    im = Image.open(im)
    h, w = im.size
...
img = Image.open('./data/images_sample/6811960/6811960_3685d3542328b820980642535d8ccb72.jpg')
ex = img._getexif()
if ex != None:
...
images = glob.glob('./data/images_sample/6812098/**.jpg')  # just comparing two folders for demo
...
for im in range(100):
    ...
    if feature < 7 and im1 != im2:
        ...
        imgx = np.concatenate(
            (Image.open(im1).resize((400, 400), Image.ANTIALIAS), Image.open(im2).resize((400, 400), Image.ANTIALIAS)),
            axis=1)
        plt.imshow(imgx);
        plt.axis('off')
        break
...
train = pd.read_json(open("./data/train.json", "r"))[:100]  # limit
y = train.interest_level.values
n = len(train)

test = pd.read_json(open("./data/test.json", "r"))[:100]  # limit
...
df_all['created'] = df_all['created'].map(
    lambda x: float((x - dt.datetime(1899, 12, 30)).days) + (float((x - dt.datetime(1899, 12, 30)).seconds) / 86400))
